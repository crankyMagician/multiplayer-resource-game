...
- **Animation library**: `assets/animations/player_animation_library.tres` — 260 animations built from UAL GLBs via `tools/build_animation_library.gd`. Track paths: `Armature/Skeleton3D:bone_name` (no remapping needed).
...
- **Locomotion mapping**: Idle→`Idle`, Jog→`Jog_Fwd`, Sprint→`Sprint`, CrouchIdle→`Crouch_Idle`, CrouchWalk→`Crouch_Fwd`, JumpUp→`Jump_Start`, Falling→`Jump`, Landing→`Jump_Land`
- **Tool action mapping** (`_get_tool_animation_name()`): `hoe`→`Farm_PlantSeed`, `axe`→`TreeChopping`, `water`→`Farm_Watering`, `harvest`→`Farm_Harvest`, `craft`→`Interact`, `fish`→`Fish_Cast`, `fish_idle`→`Fish_Cast_Idle`, `fish_hook`→`Fish_Reel`, plus 20+ additional mappings (combat, emotes, farming, climbing, etc.)
...
- **Loop modes**: Set in `build_animation_library.gd` LOOP_ANIMATIONS dict (explicit list, UAL has no suffix convention). Safety net also sets loop at runtime in `_build_animation_tree()`.
...
- **Re-build command**: `'/Applications/Mechanical Turk.app/Contents/MacOS/Mechanical Turk' --path . --script tools/build_animation_library.gd`
